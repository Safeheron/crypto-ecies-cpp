...
{
public:
    ISYMM(){cipher_ = nullptr; key_size_ = 0;};
    virtual ~ISYMM(){};
...
public:
    DESede(){cipher_ = nullptr; key_size_ = 24*8; block_size_ = 8*8;};
    virtual ~DESede(){};
public:
...
public:
    AES(size_t key_size){cipher_ = nullptr; key_size_ = key_size; block_size_ = 16*8;};
    virtual ~AES(){};
public:
    bool initKey_CBC(const unsigned char* key, size_t key_size, const unsigned char* iv, size_t iv_size);
    bool initKey_CBC(const std::string & key, const std::string &iv);
